%# -*- coding: utf-8-unix -*-
\chapter*{主要论文}
\addcontentsline{toc}{chapter}{主要论文}
\markboth{主要论文}{主要论文}

% hanglist environment is defined globally in trans_number_baker.tex

以下是与超越数\index{transcendental number}论相关的核心学术论文列表. 它包含了该领域绝大多数近年来的研究成果, 以及部分经过挑选的更具重要意义的早期专著. 关于 1967 年之前更完整的出版物列表, 参见 Feldman\index{Feldman, N. I.} 与 Shidlovsky\index{Shidlovsky, A. B.} 的综述论文（参见参考文献）.
...
I.M. 是 N.A.W. 中数学论文的汇编, 并在可能的情况下给出了前者的参考. 发表在 D.A.N. 上的数学文章通常被翻译在 Soviet Math. Doklady 中; 在适当的情况下, 翻译版本的参考会跟在等号后面. 对于发表在 Trudy Moskov.、Mat. Sb. 和 Mat. Zametki 上的文章翻译也采用了类似的惯例, 它们分别可以在 Transactions Moscow Math. Soc.、Math. USSR Sbornik 和 Math. Notes 中找到. 缩写 A.M.S.T. 表示 American Math. Soc. Translations 系列丛书, 其中包含一些来自其他期刊的文章. 原文为俄文的文献标题在本书中均给出了英文译名.
